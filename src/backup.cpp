...
int
Backup::backup(boost::filesystem::path& backup_dir, boost::filesystem::path& backup_prefix)
{	
	...
	if (boost::filesystem::exists(backup_dir, err)) {
		...
		if (boost::filesystem::is_directory(backup_dir)) {
			...
			RemoteObject::populate_remote_objects_table(remote_store, backup_prefix);
			// backup
		} else {
			return -1;
		}
